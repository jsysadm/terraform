resource "proxmox_lxc" "lxc-instance" {
  ...
  password     = var.lxc_password       # -- kintamasis faile variables.tf, lauko lxc_passwd tipas,  failas terraform.tfvars ir lxc_password  = "slaptažodis"
  unprivileged = true
  cores        = 2
  memory       = 2048
  swap         = 512
  start        = true
  ssh_public_keys = var.PUBLIC_SSH_KEY  # -- kintamasis faile variables.tf, lauko PUBLIC_SSH_KEY tipas, failas terraform.tfvars ir PUBLIC_SSH_KEY  = "pub_key"
